...
export const consumeMarketLiquidity = (
    ...
    for (const level of levels) {
        const matchesPrice =
            side === Side.BUY ? level.price <= price + 1e-8 : level.price >= price - 1e-8;
...
const getEffectiveMinOrderSize = (marketSnapshot: MarketBookSnapshot) =>
    Math.max(toSafeNumber(marketSnapshot.minOrderSize), minLimitOrderSize);
...
export const buildChunkExecutionPlan = (params: {
    ...
    if (condition === 'buy') {
        ...
        if (!isAcceptableBuyPrice(topAsk.price, sourcePrice, config)) {
            return {
                status: 'SKIPPED',
                reason: `当前买价超出允许滑点（${config.maxSlippageBps}bps）`,
                requestedSize: buyTarget.requestedUsdc / sourcePrice,
                requestedUsdc: buyTarget.requestedUsdc,
                orderAmount: 0,
        ...
        if (nextRequestedUsdc < minMarketBuyUsdc) {
            return {
                status: 'SKIPPED',
                reason: `剩余买单金额低于平台最小下单金额 ${minMarketBuyUsdc} USDC`,
        ...
        if (buyLiquidity.availableUsdc < minMarketBuyUsdc) {
            return {
                status: 'RETRYABLE_ERROR',
                reason: `盘口可成交金额不足 ${minMarketBuyUsdc} USDC`,
                requestedSize: buyTarget.requestedUsdc / sourcePrice,
                requestedUsdc: buyTarget.requestedUsdc,
                orderAmount: 0,
        ...
        if (
            ...
            return {
                status: 'RETRYABLE_ERROR',
                reason: `当前可成交金额会留下小于 ${minMarketBuyUsdc} USDC 的尾单，已暂缓执行`,
                requestedSize: buyTarget.requestedUsdc / buyLiquidity.executionPrice,
                requestedUsdc: buyTarget.requestedUsdc,
                orderAmount: 0,
                executionPrice: buyLiquidity.executionPrice,
    ...
    if (!isAcceptableSellPrice(topBid.price, sourcePrice, config)) {
        return {
            status: 'SKIPPED',
            reason: `当前卖一价格超出允许滑点（${config.maxSlippageBps}bps）`,
            requestedSize: sellTarget.requestedSize,

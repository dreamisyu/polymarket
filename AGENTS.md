...
- `npm run report:summary -- --mode trace --trace-id paper`：输出执行汇总，便于观察盈亏、失败项与结算统计。
...
当前 `npm test` 仍是占位脚本，不能作为发布依据。涉及行为变更时，至少提供一轮手工验证：准备 `.env`、启动 MongoDB、运行 `node validate-bot.js`，再执行 `npm run dev` 观察启动日志，最后用 `npm run report:summary` 检查汇总结果。当前环境在 Node `v25.3.0` 下执行全量 `npx tsc --noEmit` 仍可能 OOM，建议优先使用 Node 20 LTS，或只对改动文件做定向检查。若补充自动化测试，优先使用仓库已安装的 Jest + `ts-jest`，测试文件命名为 `*.test.ts`，放在 `src/test/` 或相邻模块目录，并优先 mock Polymarket 与 MongoDB 依赖。

...
- 路径导入统一使用路径别名。
- 公共节点禁止通过 `strategyKind` 或策略专属配置分支承载定制逻辑；一旦某个策略需要特殊规则，必须新增专属节点并由策略自己编排到工作流。
- 共享 workflow builder 必须暴露阶段级可替换插槽（如 sizing、trade planning、merge planning），不要把某个策略当前使用的节点硬编码成所有策略的默认事实源。
- 判断“某段逻辑是否应放入公共节点”的标准不是“有没有别的策略暂时也能复用”，而是“该逻辑是否对策略语义无感”；只要规则依赖某个策略的独占意图，就应下沉到该策略的专属节点。
